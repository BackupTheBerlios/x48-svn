...
int             Memory;
int             Trace;
word_20         MemoryAddress = 0;
struct objaddr_t *ObjectAddress;
int             Stack;
int		Disassembly;
int             Disrpl;
...
#define MENUOFFSET_Y            (TOP_SKIP+16)

#define REGSIZE_Y               500;
#define REG_DISPLAY_Y           240;
...
color_t colors_sx[] = {
  ...
  { "black", 0, 0, 0, 0, 0,
    { 0, 0, 0, 0, DoRed | DoGreen | DoBlue, 0 } },
  { "grey", 127, 127, 127, 127, 127,
    { 0, 0, 0, 0, DoRed | DoGreen | DoBlue, 0 } },
  { "red", 255, 0, 0, 127, 127,
    { 0, 0, 0, 0, DoRed | DoGreen | DoBlue, 0 } },
  { "green", 0, 255, 0, 127, 127, 
    { 0, 0, 0, 0, DoRed | DoGreen | DoBlue, 0 } },
  { "yellow", 255, 255, 0, 127,127,
    { 0, 0, 0, 0, DoRed | DoGreen | DoBlue, 0 } },
  { "blue", 0, 0, 255, 127, 127,
    { 0, 0, 0, 0, DoRed | DoGreen | DoBlue, 0 } },
  { 0 }
};
...
#endif
{
  ...
  for (i = 0; i < length; i++) {
    if (small_font[(int)string[i]].h != 0) {
      ...
      if (!quiet)
        fprintf(stderr, "Unknown small letter '%c' ; code: %i\n", string[i], (int)string[i]);
      w += 5;
    }
  }
  return w;
...
int
#ifdef __FunctionProto__
CreateButtons(int first_button, int last_button, Window parent)
#else
CreateButtons(first_button, last_button, parent)
  int first_button;
  int last_button;
  Window parent;
#endif
{
  ...
  for (i = first_button; i <= last_button; i++) {
    ...
    if (i < BUTTON_MTH && ( buttons == buttons_sx || buttons == buttons_gx ))
     ...
     */
    if (buttons == buttons_sx || buttons == buttons_gx)
      buttons[i].xwin = XCreateSimpleWindow(dpy, parent,
...
int
#ifdef __FunctionProto__
DrawButtons(int first, int last, struct button_t *but)
#else
DrawButtons(first, last, but)
  int first;
  int last;
  struct button_t *but;
#endif
{
  ...
  for (i = first; i <= last; i++) {
    if (but[i].pressed) {
      XCopyArea(dpy, but[i].down, but[i].xwin, gc, 0, 0, but[i].w,
                but[i].h, 0, 0);
    } else {
      XCopyArea(dpy, but[i].map, but[i].xwin, gc, 0, 0, but[i].w,
                but[i].h, 0, 0);
    }
  }
  return 0;
...
int
#ifdef __FunctionProto__
DrawButton(int i, struct button_t *but)
#else
DrawButton(i, but)
  int i;
  struct button_t *but;
...
int
#ifdef __FunctionProto__
CreateKeypad(unsigned int w, unsigned int h, Window parent, 
                                struct keypad_t *pad, int first, int last)
#else
CreateKeypad(w, h, parent, pad, first, last)
unsigned int w;
unsigned int h;
Window parent;
struct keypad_t *pad;
...
#endif
{
  ...
  unsigned int  pw, ph;
  unsigned int  display_offset_x=0, display_offset_y=0, display_height=0;
  ...
  if ( parent == regWin )
    pad->offset = MENUOFFSET_Y + BOTTOM_SKIP + buttons_menu[MBUTTON_REGDONE].y +
          buttons_menu[MBUTTON_REGDONE].h;
  if ( parent == stackWin )
    pad->offset = MENUOFFSET_Y + BOTTOM_SKIP + buttons_menu[MBUTTON_STDONE].y +
          buttons_menu[MBUTTON_STDONE].h;
  if ( parent == asmWin )
    pad->offset = MENUOFFSET_Y + BOTTOM_SKIP + buttons_menu[MBUTTON_ASMDONE].y +
          buttons_menu[MBUTTON_ASMDONE].h;
  if ( parent == ramWin )
    pad->offset = MENUOFFSET_Y + BOTTOM_SKIP + buttons_menu[MBUTTON_RAMDONE].y +
          buttons_menu[MBUTTON_RAMDONE].h;
  if ( parent == objWin ) 
    pad->offset = MENUOFFSET_Y + BOTTOM_SKIP + buttons_menu[MBUTTON_OBJDONE].y +
          buttons_menu[MBUTTON_OBJDONE].h;


  pad->pixmap = XCreatePixmap(dpy, parent, w, h, depth);

  XSetBackground(dpy, gc, COLOR(PAD));
  XSetForeground(dpy, gc, COLOR(PAD));

  XFillRectangle(dpy, pad->pixmap, gc, 0, 0, w, h);
  ...
  if (parent == mainW)
    ...
    cut = MENUOFFSET_Y +  buttons[0].y - 19;

  XFillRectangle(dpy, pad->pixmap, gc, 0, 0, w, (unsigned int)cut);
  ...
  for (i = first; i <= last; i++) {
    ...
    if (buttons[i].letter != (char *)0) {
      ...
      DrawSmallString(dpy, pad->pixmap, gc, x, y,
                      buttons[i].letter, 1);
    }
  }
  ...
  for (i = first; i <= last; i++) {
	...
	pix = XCreatePixmap(dpy, pad->pixmap, pw, ph, depth);
	...
	x = (pw + 1 - SmallTextWidth(buttons[i].left,
      ...
      if(parent != mainW)
        ...
        XCopyArea(dpy, pix, pad->pixmap, gc, 0, 0, pw, ph, x, y);
  ...
  pix = XCreateBitmapFromData(dpy, pad->pixmap, (char *)hp_bits,
                              hp_width, hp_height);
  ...
  XCopyPlane(dpy, pix, pad->pixmap, gc, 0, 0, hp_width, hp_height,
             x, 10, 1);
  ...
  if (!opt_gx)
    {
      ...
      XDrawLine(dpy, pad->pixmap, gc, (int)SIDE_SKIP, 9,
                (int)(SIDE_SKIP + hp_width - 1), 9);
      XDrawLine(dpy, pad->pixmap, gc, (int)(SIDE_SKIP - 1), 10,
                (int)(SIDE_SKIP - 1), 10 + hp_height - 1);
      XDrawLine(dpy, pad->pixmap, gc, (int)SIDE_SKIP, 10 + hp_height,
                (int)(SIDE_SKIP + hp_width - 1), 10 + hp_height);
      XDrawLine(dpy, pad->pixmap, gc, (int)(SIDE_SKIP + hp_width), 10,
                (int)(SIDE_SKIP + hp_width), 10 + hp_height - 1);
    }
  ...
  if (opt_gx)
    {
      x = SIDE_SKIP + DISPLAY_WIDTH - gx_128K_ram_width
          + gx_128K_ram_x_hot + 2;
      y = 10 + gx_128K_ram_y_hot;
      pix = XCreateBitmapFromData(dpy, pad->pixmap, (char *)gx_128K_ram_bits,
                                  gx_128K_ram_width, gx_128K_ram_height);
      XCopyPlane(dpy, pix, pad->pixmap, gc, 0, 0, gx_128K_ram_width,
                 gx_128K_ram_height, x, y, 1);
      XFreePixmap(dpy, pix);

      XSetForeground(dpy, gc, COLOR(LOGO));
      x = SIDE_SKIP + hp_width;
      y = hp_height + 8 - hp48gx_height;
      pix = XCreateBitmapFromData(dpy, pad->pixmap, (char *)hp48gx_bits,
                                  hp48gx_width, hp48gx_height);
      XCopyPlane(dpy, pix, pad->pixmap, gc, 0, 0, hp48gx_width,
                 hp48gx_height, x, y, 1);
      XFreePixmap(dpy, pix);
      ...
      pix = XCreateBitmapFromData(dpy, pad->pixmap, (char *)gx_silver_bits,
                                  gx_silver_width, gx_silver_height);
      XSetStipple(dpy, gc, pix);
      XSetTSOrigin(dpy, gc, x, y);
      XFillRectangle(dpy, pad->pixmap, gc, x, y, gx_silver_width,
                     gx_silver_height);
      XFreePixmap(dpy, pix);
      ...
      x = SIDE_SKIP + DISPLAY_WIDTH - gx_128K_ram_width
          + gx_green_x_hot + 2;
      y = 10 + gx_green_y_hot;
      pix = XCreateBitmapFromData(dpy, pad->pixmap, (char *)gx_green_bits,
                                  gx_green_width, gx_green_height);
      XSetStipple(dpy, gc, pix);
      XSetTSOrigin(dpy, gc, x, y);
      XFillRectangle(dpy, pad->pixmap, gc, x, y, gx_green_width,
                     gx_green_height);
      XFreePixmap(dpy, pix);
  ...
  else
    {
      ...
      pix = XCreateBitmapFromData(dpy, pad->pixmap, (char *)hp48sx_bits,
                                  hp48sx_width, hp48sx_height);

      XCopyPlane(dpy, pix, pad->pixmap, gc, 0, 0, hp48sx_width,
                 hp48sx_height, x, y, 1);

      XFreePixmap(dpy, pix);

      x = SIDE_SKIP + DISPLAY_WIDTH - 1 - science_width;
      y = TOP_SKIP - DISP_FRAME - science_height - 4;

      pix = XCreateBitmapFromData(dpy, pad->pixmap, (char *)science_bits,
                                  science_width, science_height);

      XCopyPlane(dpy, pix, pad->pixmap, gc, 0, 0, science_width,
  ...
  if(parent == mainW ) {
    ...
    display_offset_x = SIDE_SKIP;
    display_offset_y = pad->offset;
    display_height =  pad->height - BOTTOM_SKIP - display_offset_y; 
    display_width = w - (2*SIDE_SKIP);
  } 

  XSetForeground(dpy, gc, COLOR(DISP_PAD_TOP));

  for (i = 0; i < DISP_FRAME; i++) {
    XDrawLine(dpy, pad->pixmap, gc,
  ...
  XDrawLine(dpy, pad->pixmap, gc,
            ...
            (int)(display_offset_x + display_width),
            (int)(display_offset_y + display_height - 2));
  XFillRectangle(dpy, pad->pixmap, gc, 
               display_offset_x,
               display_offset_y,
               display_width,
               display_height );
  ...
  if (!opt_gx)
    {
      ...
      pix = XCreateBitmapFromData(dpy, pad->pixmap, (char *)last_bits,
      ...
      XCopyPlane(dpy, pix, pad->pixmap, gc, 0, 0, last_width, last_height,
	         x, y, 1);
  ...
  /* bottom lines */
  XDrawLine(dpy, pad->pixmap, gc, 1, (int)(pad->height - 1),
	    (int)(pad->width - 1), (int)(pad->height - 1));
  XDrawLine(dpy, pad->pixmap, gc, 2, (int)(pad->height - 2),
	    (int)(pad->width - 2), (int)(pad->height - 2));

  /* right lines */
  XDrawLine(dpy, pad->pixmap, gc, (int)(pad->width - 1),
	    (int)(pad->height - 1), (int)(pad->width - 1), cut);
  XDrawLine(dpy, pad->pixmap, gc, (int)(pad->width - 2),
	    (int)(pad->height - 2), (int)(pad->width - 2), cut);

  XSetForeground(dpy, gc, COLOR(DISP_PAD_TOP));

  /* right lines */
  XDrawLine(dpy, pad->pixmap, gc, (int)(pad->width - 1),
	    cut - 1, (int)(pad->width - 1), 1);
  XDrawLine(dpy, pad->pixmap, gc, (int)(pad->width - 2),
	    cut - 1, (int)(pad->width - 2), 2);

  XSetForeground(dpy, gc, COLOR(DISP_PAD_BOT));

  /* top lines */
  XDrawLine(dpy, pad->pixmap, gc, 0, 0, (int)(pad->width - 2), 0);
  XDrawLine(dpy, pad->pixmap, gc, 1, 1, (int)(pad->width - 3), 1);

  /* left lines */
  XDrawLine(dpy, pad->pixmap, gc, 0, cut - 1, 0, 0);
  XDrawLine(dpy, pad->pixmap, gc, 1, cut - 1, 1, 1);

  XSetForeground(dpy, gc, COLOR(PAD_BOT));

  /* left lines */
  XDrawLine(dpy, pad->pixmap, gc, 0, (int)(pad->height - 2), 0, cut);
  XDrawLine(dpy, pad->pixmap, gc, 1, (int)(pad->height - 3), 1, cut);
  ...
  /* bottom lines */
  XDrawLine(dpy, pad->pixmap, gc, 3, (int)(pad->height - 3),
	    (int)(pad->width - 3), (int)(pad->height - 3));
  XDrawLine(dpy, pad->pixmap, gc, 4, (int)(pad->height - 4),
	    (int)(pad->width - 4), (int)(pad->height - 4));

  /* right lines */
  XDrawLine(dpy, pad->pixmap, gc, (int)(pad->width - 3),
	    (int)(pad->height - 3), (int)(pad->width - 3), cut);
  XDrawLine(dpy, pad->pixmap, gc, (int)(pad->width - 4),
	    (int)(pad->height - 4), (int)(pad->width - 4), cut);

  XSetForeground(dpy, gc, COLOR(DISP_PAD_TOP));

  /* right lines */
  XDrawLine(dpy, pad->pixmap, gc, (int)(pad->width - 3),
	    cut - 1, (int)(pad->width - 3), KEYBOARD_OFFSET_Y - 24);
  XDrawLine(dpy, pad->pixmap, gc, (int)(pad->width - 4),
	    cut - 1, (int)(pad->width - 4), KEYBOARD_OFFSET_Y - 23);

  XSetForeground(dpy, gc, COLOR(DISP_PAD_BOT));

  /* top lines */
  XDrawLine(dpy, pad->pixmap, gc, 2, KEYBOARD_OFFSET_Y - 25,
            (int)(pad->width - 4), KEYBOARD_OFFSET_Y - 25);
  XDrawLine(dpy, pad->pixmap, gc, 3, KEYBOARD_OFFSET_Y - 24,
            (int)(pad->width - 5), KEYBOARD_OFFSET_Y - 24);

  /* left lines */
  XDrawLine(dpy, pad->pixmap, gc, 2, cut - 1, 2, KEYBOARD_OFFSET_Y - 24);
  XDrawLine(dpy, pad->pixmap, gc, 3, cut - 1, 3, KEYBOARD_OFFSET_Y - 23);

  XSetForeground(dpy, gc, COLOR(PAD_BOT));

  /* left lines */
  XDrawLine(dpy, pad->pixmap, gc, 2, (int)(pad->height - 4), 2, cut);
  XDrawLine(dpy, pad->pixmap, gc, 3, (int)(pad->height - 5), 3, cut);
  ...
  /* bottom lines */
  XDrawLine(dpy, pad->pixmap, gc, 5, (int)(pad->height - 5),
	    (int)(pad->width - 3), (int)(pad->height - 5));
  XDrawLine(dpy, pad->pixmap, gc, 6, (int)(pad->height - 6),
	    (int)(pad->width - 4), (int)(pad->height - 6));

  /* right lines */
  XDrawLine(dpy, pad->pixmap, gc, (int)(pad->width - 5),
	    (int)(pad->height - 5), (int)(pad->width - 5), cut + 1);
  XDrawLine(dpy, pad->pixmap, gc, (int)(pad->width - 6),
	    (int)(pad->height - 6), (int)(pad->width - 6), cut + 2);

  XSetForeground(dpy, gc, COLOR(DISP_PAD_BOT));

  /* top lines */
  XDrawLine(dpy, pad->pixmap, gc, 4, cut,
            (int)(pad->width - 6), cut);
  XDrawLine(dpy, pad->pixmap, gc, 5, cut + 1,
            (int)(pad->width - 7), cut + 1);

  XSetForeground(dpy, gc, COLOR(PAD_BOT));

  /* left lines */
  XDrawLine(dpy, pad->pixmap, gc, 4, (int)(pad->height - 6), 4, cut + 1);
  XDrawLine(dpy, pad->pixmap, gc, 5, (int)(pad->height - 7), 5, cut + 2);
  ...
  XDrawLine(dpy, pad->pixmap, gc, (int)(pad->width - 7),
            (int)(pad->height - 7), (int)(pad->width - 7),
            (int)(pad->height - 14));
  XDrawLine(dpy, pad->pixmap, gc, (int)(pad->width - 8),
            (int)(pad->height - 8), (int)(pad->width - 8),
            (int)(pad->height - 11));
  XDrawLine(dpy, pad->pixmap, gc, (int)(pad->width - 7),
            (int)(pad->height - 7), (int)(pad->width - 14),
            (int)(pad->height - 7));
  XDrawLine(dpy, pad->pixmap, gc, (int)(pad->width - 7),
            (int)(pad->height - 8), (int)(pad->width - 11),
            (int)(pad->height - 8));
  XDrawPoint(dpy, pad->pixmap, gc,
             (int)(pad->width - 9), (int)(pad->height - 9));

  XDrawLine(dpy, pad->pixmap, gc, 7, (int)(pad->height - 7), 13,
            (int)(pad->height - 7));
  XDrawLine(dpy, pad->pixmap, gc, 8, (int)(pad->height - 8), 10,
            (int)(pad->height - 8));
  XSetForeground(dpy, gc, COLOR(PAD_BOT));
  XDrawLine(dpy, pad->pixmap, gc, 6, (int)(pad->height - 8), 6,
            (int)(pad->height - 14));
  XDrawLine(dpy, pad->pixmap, gc, 7, (int)(pad->height - 9), 7,
            (int)(pad->height - 11));
...
int
#ifdef __FunctionProto__
DrawKeypad(struct keypad_t src, Drawable dest)
#else
DrawKeypad(src, dest)
  Drawable src;
  Drawable dest;
#endif
{
  XCopyArea(dpy, src.pixmap, dest, gc, 0, 0, src.width,
            src.height, 0, 0);
  return 0;
}
...
#endif
{
  ...
  unsigned long gc_mask;
  unsigned int	mask;
  XSetWindowAttributes xswa, stwa;
  XTextProperty	wname, iname;
  Atom		protocols[2];
  char		*name, *user_geom, def_geom[40];
  int		info, x, y, w, h;
  unsigned int	width, height, regheight;
  ...
  menuWin = XCreateWindow( dpy, RootWindow(dpy, screen),
                        0, 0, width, MENUOFFSET_Y + buttons[MBUTTON_MENUDONE].y,
                        0, (int)depth, class,
                        visual, 0, &stwa);

  regWin = XCreateWindow( dpy, RootWindow(dpy, screen),
                        0, 0, width, regheight,
                        0, (int)depth, class,
                        visual, 0, &stwa);
  stackWin = XCreateWindow( dpy, RootWindow(dpy, screen),
                        0, 0, width, regheight,
                        0, (int)depth, class,
                        visual, 0, &stwa);
  ramWin = XCreateWindow( dpy, RootWindow(dpy, screen),
                        0, 0, width, regheight,
                        0, (int)depth, class,
                        visual, 0, &stwa);
  asmWin = XCreateWindow( dpy, RootWindow(dpy, screen),
                        0, 0, width * 2, regheight,
                        0, (int)depth, class,
                        visual, 0, &stwa);

  objWin = XCreateWindow( dpy, RootWindow(dpy, screen),
                        0, 0, width, regheight,
                        0, (int)depth, class,
                        visual, 0, &stwa);

  if (mainW == 0 || menuWin == 0 || regWin == 0 || stackWin == 0 || 
      asmWin == 0 || ramWin == 0 || objWin == 0 )
    return -1;
   ...
   * parse -geometry ...
   */
  mhint.x = mhint.y = hint.x = hint.y = 0;
  ...
  mhint.min_width = mhint.max_width = mhint.base_width = mhint.width = width;
  mhint.min_height = mhint.max_height =  mhint.base_height = mhint.height = 
     MENUOFFSET_Y + BOTTOM_SKIP + buttons[MBUTTON_MENUDONE].y + 
     buttons[MBUTTON_MENUDONE].h; 

  hint.win_gravity = NorthWestGravity;
  hint.flags = PSize | PMinSize | PMaxSize | PBaseSize | PWinGravity;
  mhint.win_gravity = NorthWestGravity;
  mhint.flags = PSize | PMinSize | PMaxSize | PBaseSize | PWinGravity;
  ...
  stwa.background_pixel = COLOR(PAD);
  stwa.border_pixel = COLOR(BLACK);
  stwa.backing_store = Always; 
  stwa.win_gravity = mhint.win_gravity;
  stwa.bit_gravity = NorthWestGravity; 
  stwa.event_mask = GCForeground | GCLineWidth | GCBackground; /* GCFont */
  XChangeWindowAttributes(dpy, menuWin, mask, &stwa); 
  XMoveWindow(dpy, menuWin, hint.x + 100 , hint.y);
  XChangeWindowAttributes(dpy, regWin, mask, &stwa);
  XMoveWindow(dpy, regWin, hint.x + 100 , hint.y);
  XChangeWindowAttributes(dpy, stackWin, mask, &stwa);
  XMoveWindow(dpy, stackWin, hint.x + 100 , hint.y);
  XChangeWindowAttributes(dpy, asmWin, mask, &stwa);
  XMoveWindow(dpy, asmWin, hint.x + 100 , hint.y);
  XChangeWindowAttributes(dpy, objWin, mask, &stwa);
  XMoveWindow(dpy, objWin, hint.x + 100 , hint.y);
  XChangeWindowAttributes(dpy, ramWin, mask, &stwa);
  XMoveWindow(dpy, ramWin, hint.x + 100 , hint.y);
  ...
  sprintf(name, "%s-%d.%d.%d Menu", progname, saturn.version[0],
              saturn.version[1], saturn.version[2]);
  XStringListToTextProperty(&name, 1, &wname);
  XSetWMProperties(dpy, menuWin, &wname, &iname, argv, argc, 
                                                     &mhint, &wmh, &clh);

  mhint.min_height = mhint.max_height =  mhint.base_height = mhint.height = regheight;
  sprintf(name, "%s-%d.%d.%d Registers", progname, saturn.version[0],
              saturn.version[1], saturn.version[2]);
  XStringListToTextProperty(&name, 1, &wname);
  XSetWMProperties(dpy, regWin, &wname, &iname, argv, argc,
                                                     &mhint, &wmh, &clh);
  sprintf(name, "%s-%d.%d.%d Stack", progname, saturn.version[0],
              saturn.version[1], saturn.version[2]);
  XStringListToTextProperty(&name, 1, &wname);
  XSetWMProperties(dpy, stackWin, &wname, &iname, argv, argc,
                                                     &mhint, &wmh, &clh);
  sprintf(name, "%s-%d.%d.%d Memory", progname, saturn.version[0],
              saturn.version[1], saturn.version[2]);
  XStringListToTextProperty(&name, 1, &wname);
  XSetWMProperties(dpy, ramWin, &wname, &iname, argv, argc,
                                                     &mhint, &wmh, &clh);

  sprintf(name, "%s-%d.%d.%d Object", progname, saturn.version[0],
              saturn.version[1], saturn.version[2]);
  XStringListToTextProperty(&name, 1, &wname);
  XSetWMProperties(dpy, objWin, &wname, &iname, argv, argc,
                                                     &mhint, &wmh, &clh);

  mhint.min_width = mhint.max_width = mhint.base_width = mhint.width = 2*width;
  sprintf(def_geom, "%ux%u", 2*width, height);
  user_geom = get_string_resource("geometry", "Geometry");

  sprintf(name, "%s-%d.%d.%d Debugger", progname, saturn.version[0],
              saturn.version[1], saturn.version[2]);
  XStringListToTextProperty(&name, 1, &wname);
  XSetWMProperties(dpy, asmWin, &wname, &iname, argv, argc,
                                                     &mhint, &wmh, &clh);
   ...
   * set up the GC's
   */
  vals.background = COLOR(LCD);
  vals.foreground = COLOR(BLACK);
  vals.function = GXcopy;
  DispFont = get_font_resource(dpy,  "smallLabelFont", "SmallLabelFont");
  vals.font = DispFont->fid;
  font_height = DispFont->ascent + DispFont->descent;
  gc_mask = GCForeground | GCBackground | GCFunction | GCFont ;
  gc_stack = XCreateGC(dpy, menuWin, gc_mask, &vals);

  vals.background = COLOR(BLACK);
  vals.foreground = COLOR(LCD);
  gc_stack_erase = XCreateGC(dpy, menuWin, gc_mask, &vals);
   ...
   * create the buttons
   */
  CreateButtons(BUTTON_A, LAST_BUTTON, mainW);

  /*
   * create the keypad
   */
  CreateKeypad(width, height, mainW, &keypad, BUTTON_A, LAST_BUTTON);
  /*
   * map the window
   */
  ...
  DrawKeypad( keypad, mainW);
  DrawButtons(BUTTON_A, LAST_BUTTON, buttons);

  DrawIcon();

  buttons = buttons_menu; 

  CreateButtons(MBUTTON_MENUSTART, MBUTTON_MENUDONE, menuWin); 
  CreateKeypad(width, width, menuWin, &menupad, MBUTTON_MENUSTART, MBUTTON_MENUDONE); 
  DrawKeypad (menupad, menuWin);
  DrawButtons(MBUTTON_MENUSTART, MBUTTON_MENUDONE, buttons_menu);

  CreateButtons(MBUTTON_REGSTART, MBUTTON_REGDONE, regWin);
  CreateKeypad(width, regheight, regWin, &regpad, MBUTTON_REGSTART, MBUTTON_REGDONE);
  DrawKeypad (regpad, regWin);
  DrawButtons(MBUTTON_REGSTART, MBUTTON_REGDONE, buttons_menu);

  CreateButtons(MBUTTON_STSTART, MBUTTON_STDONE, stackWin);
  CreateKeypad(width, regheight, stackWin, &stackpad, MBUTTON_STSTART, MBUTTON_STDONE);
  DrawKeypad (stackpad, stackWin);
  DrawButtons(MBUTTON_STSTART, MBUTTON_STDONE, buttons_menu);

  CreateButtons(MBUTTON_ASMSTART, MBUTTON_ASMDONE, asmWin);
  CreateKeypad(2*width, regheight, asmWin, &asmpad, MBUTTON_ASMSTART, MBUTTON_ASMDONE);
  DrawKeypad (asmpad, asmWin);
  DrawButtons(MBUTTON_ASMSTART, MBUTTON_ASMDONE, buttons_menu);

  CreateButtons(MBUTTON_RAMSTART, MBUTTON_RAMDONE, ramWin);
  CreateKeypad(width, regheight, ramWin, &rampad, MBUTTON_RAMSTART,
                MBUTTON_RAMDONE);
  DrawKeypad (rampad, ramWin);
  DrawButtons(MBUTTON_RAMSTART, MBUTTON_RAMDONE, buttons_menu);

  CreateButtons(MBUTTON_OBJSTART, MBUTTON_OBJDONE, objWin);
  CreateKeypad(width, regheight, objWin, &objpad, MBUTTON_OBJSTART,
                MBUTTON_OBJDONE);
  DrawKeypad (objpad, objWin);
  DrawButtons(MBUTTON_OBJSTART, MBUTTON_OBJDONE, buttons_menu);
...
#endif
{
  ...
  send.xbutton.window = mainW;
  send.xbutton.button = Button1;
  send.xbutton.subwindow = buttons[BUTTON_ON].xwin;

  XSendEvent( dpy, mainW, False, mask, &send);
...
int
#ifdef __FunctionProto__
XMoveWin(struct keypad_t d, int line )
#else
XMoveWin(d, line)
  Drawable  d;
  int       line;
#endif
{
  int                  height, indent, offset, win_width ;

  height = font_height;
  indent = SIDE_SKIP ;
  offset = d.offset;
  win_width = d.width - 2 * (indent);
  XSetForeground(dpy, gc_stack, COLOR(BLACK));
  XSetBackground(dpy, gc_stack, COLOR(LCD));

  XCopyArea(dpy, d.pixmap, d.pixmap, gc_stack,
              indent, offset + height ,
              win_width, height*line + DispFont->descent ,
              indent, offset);
...
#endif
{
  ...
  height =  ((d.height - d.offset - font_height) / font_height) -1;
  return height;
}

int
#ifdef __FunctionProto__
XDrawStr(struct keypad_t d, int line, char *buf, int normal )
#else
XDrawStr(d, line, buf, normal)
  struct keypad_t  d;
  int       line;
  char      *buf;
  int       normal;
#endif
{
  int                  height, indent, offset, coffset, win_width, clear_from;

  height = font_height;
  indent = SIDE_SKIP ;
  offset = d.offset;
  coffset = offset + DispFont->descent;
  win_width = d.width - 2 * (indent);
  ...
  if (normal) {
    ...
    XSetForeground(dpy, gc_stack, COLOR(LCD));
    XSetBackground(dpy, gc_stack, COLOR(BLACK));
  }

  XDrawImageString( dpy, d.pixmap, gc_stack,
            indent, line * height + offset, buf, strlen(buf) );
  clear_from = XTextWidth(DispFont, buf, strlen(buf)) + indent;
  if (normal)
    XFillRectangle (dpy, d.pixmap, gc_stack_erase,
            clear_from, (line - 1)*height + coffset,
            win_width - clear_from + indent, height);
  else {
    XSetForeground(dpy, gc_stack, COLOR(BLACK));
    XFillRectangle (dpy, d.pixmap, gc_stack,
            clear_from, (line - 1)*height + coffset,
            win_width - clear_from + indent, height);
  }
...
void
#ifdef __FunctionProto__
XClearBot(struct keypad_t d, int line)
#else
XClearBot(d, line )
  struct keypad_t d;
  int       line;
#endif
{
  ...
  int                  height, indent, offset, coffset, win_width ;
  ...
  line--;
  XFillRectangle(dpy, d.pixmap, gc_stack,
           indent, offset +(line*height) + DispFont->descent,
           win_width,  d.height - (offset +(line*height) + 
                        DispFont->descent + BOTTOM_SKIP) );

}

int
#ifdef __FunctionProto__
update_stack(struct se *stack, int n )
#else
...
#endif
{
  int     i=n, offset, coffset , str, j, k, l, start=0;
  word_20 len;
  char    buf[65536], buf1[100], buf2[100];
  int     clear_from;
  int     indent, height;
  ...
  height = font_height;
  indent = SIDE_SKIP ;
  offset = REG_DISPLAY_Y;
  coffset = DispFont->descent;
  win_width = KEYBOARD_WIDTH;
  win_height = REGSIZE_Y ; 
  XSetForeground(dpy, gc_stack, COLOR(BLACK));
  XSetBackground(dpy, gc_stack, COLOR(LCD));

  str = 24;
  if ( !buttons_menu[MBUTTON_STADDR].code )
    str += 9;
  if ( !buttons_menu[MBUTTON_STTYPE].code)
    str +=16;
  i = 15;
  for (k = 0; k < n; k++)
    {
      ...
      decode_rpl_obj(stack->se_p, &len, buf);
      if (strlen(buf) > str+3) {
        strncpy(buf1, buf, str+3);
        sprintf(&buf1[str], "...");
        buf1[str+3] = '\0';
       if ( buttons_menu[MBUTTON_STMORE].code <= (strlen(buf) - str )/33 +1) 
         i -= buttons_menu[MBUTTON_STMORE].code ;
       else
         i -= (strlen(buf) - str )/33 +1;
       start = i;
       }
       else
         strcpy(buf1, buf);
      ...
      if ( i >= 0) {
        if (buttons_menu[MBUTTON_STADDR].code &&
                                            buttons_menu[MBUTTON_STTYPE].code)
           sprintf(&buf2[0], "%2d: %.5lX -> %s", stack->se_n, stack->se_p, buf1);
        if ( !buttons_menu[MBUTTON_STADDR].code && 
                 buttons_menu[MBUTTON_STTYPE].code)
           sprintf(&buf2[0], "%2d: %s", stack->se_n, buf1);
        if ( !buttons_menu[MBUTTON_STADDR].code &&
              !buttons_menu[MBUTTON_STTYPE].code)
           sprintf(&buf2[0], "%2d: %s", stack->se_n, &buf1[16]);
        if (buttons_menu[MBUTTON_STADDR].code &&
             !buttons_menu[MBUTTON_STTYPE].code)
           sprintf(&buf2[0], "%2d: %.5lX -> %s", stack->se_n, 
                                                      stack->se_p, &buf1[16]);

        XDrawImageString( dpy, stackWin, gc_stack,
                        indent , offset  + (i*height), buf2, strlen(buf2));

        clear_from = XTextWidth(DispFont, buf2, strlen(buf2)) ;
        XFillRectangle (dpy, stackWin, gc_stack_erase,
             indent + clear_from, offset +  ((i-1)*height) +coffset ,
             win_width - clear_from, height);


        if (strlen(buf)>str && buttons_menu[MBUTTON_STMORE].code) 
          {
            l = 0;
            for(j=0; j < buttons_menu[MBUTTON_STMORE].code; j++) {
              i++;
              strncpy(buf1,&buf[str + j*33],60);
              if (strlen(buf1) > 36) {
                  sprintf(&buf1[33], "...");
                  buf1[36] = '\0';
               }
...
#endif
{
  switch (i) {
    case MBUTTON_RAMUP1:
      MemoryAddress += 1;
      break;
    case MBUTTON_RAMUP10:
      MemoryAddress += 0x10;
      break;
    case MBUTTON_RAMUP100:
      MemoryAddress += 0x100;
      break;
    case MBUTTON_RAMUP1000:
      MemoryAddress += 0x1000;
      break;
    case MBUTTON_RAMUP10000:
      MemoryAddress += 0x10000;
      break;
    case MBUTTON_RAMDN1:
      MemoryAddress -= 1;
      break;
    case MBUTTON_RAMDN10:
      MemoryAddress -= 0x10;
      break;
    case MBUTTON_RAMDN100:
      MemoryAddress -= 0x100;
      break;
    case MBUTTON_RAMDN1000:
      MemoryAddress -= 0x1000;
      break;
    case MBUTTON_RAMDN10000:
      MemoryAddress -= 0x10000;
      break;
    case MBUTTON_RAMHOME:
      MemoryAddress = read_nibbles(0x70592,5);
      break;
    case MBUTTON_RAMPROLOG:
    case MBUTTON_RAMSRPL:
      if (buttons_menu[i].code)
        buttons_menu[i].code = 0;
      else
        buttons_menu[i].code = 1;
      break;
    default:
      if ( but == Button1 ) {
        switch (i) {
          case MBUTTON_RAMREGA:
            MemoryAddress = (saturn.A[4] << 16 ) + (saturn.A[3] << 12) +
                            (saturn.A[2] << 8) + (saturn.A[1] << 4 ) +
                             saturn.A[0];
            break;
          case MBUTTON_RAMREGB:
            MemoryAddress = (saturn.B[4] << 16 ) + (saturn.B[3] << 12) +
                            (saturn.B[2] << 8) + (saturn.B[1] << 4 ) +
                             saturn.B[0];
            break;
          case MBUTTON_RAMREGC:
            MemoryAddress = (saturn.C[4] << 16 ) + (saturn.C[3] << 12) +
                            (saturn.C[2] << 8) + (saturn.A[1] << 4 ) +
                             saturn.C[0];
            break;
          case MBUTTON_RAMREGD:
            MemoryAddress = (saturn.D[4] << 16 ) + (saturn.D[3] << 12) +
                            (saturn.D[2] << 8) + (saturn.A[1] << 4 ) +
                             saturn.D[0];
            break;
          case MBUTTON_RAMREGD0:
            MemoryAddress = saturn.D0;
            break;
          case MBUTTON_RAMREGD1:
            MemoryAddress = saturn.D1;
            break;
        }
      } else {
        if (buttons_menu[i].code)
...
{ 
  switch (i) {
    case (MBUTTON_STMORE):
      buttons_menu[MBUTTON_STMORE].code++;
      break;
    case ( MBUTTON_STLESS):
      if (buttons_menu[MBUTTON_STMORE].code > 0)
        buttons_menu[MBUTTON_STMORE].code--;
      break;
    default:
      if (buttons_menu[i].code)
...
#endif
{
  ...
  switch (i) { 
    case MBUTTON_OBJHOME: 
      if (buttons_menu[i].code) {
        ObjectAddress = (struct objaddr_t *)malloc(sizeof(struct objaddr_t));
        ObjectAddress->addr = read_nibbles(0x70592, 5);
        ObjectAddress->obj_next = (struct objaddr_t *)0;
      } else {
        free(ObjectAddress);
      }
      break;
    case MBUTTON_OBJPORT0:
      if (buttons_menu[i].code) {
        ObjectAddress = (struct objaddr_t *)malloc(sizeof(struct objaddr_t));
        ObjectAddress->addr = read_nibbles(0x70597, 5) -5;
        ObjectAddress->obj_next = (struct objaddr_t *)0;
      } else {
        free(ObjectAddress);
      }
...
#endif
{
  ...
  XSetBackground(dpy, gc, COLOR(LOGO_BACK));
  XSetForeground(dpy, gc, COLOR(i));

  pix = XCreateBitmapFromData(dpy, menuWin, (char *)hp_bits,
                              hp_width, hp_height);
...
int
#ifdef __FunctionProto__
PopUpBreakRegister(int i)
#else
PopUpBreakRegister(i)
  int i;
#endif
{
  char buf[100];
  int  offset_x, offset_y1, offset_y2, offset_y3, cell_x, cell_y;
  int  win_width, win_hieght;
  int  j;
  unsigned int  mask;
  XSetWindowAttributes stwa;
  struct bkreg_t *op;
  char    fmt[20];
  word_20   k;

  offset_x = (int)SIDE_SKIP;
  offset_y1 = 1.5* font_height + DispFont->descent;
  offset_y2 = 3.5* font_height + DispFont->descent;
  offset_y3 = 5  * font_height + DispFont->descent;
  cell_y = font_height + DispFont->descent;
  cell_x = XTextWidth(DispFont,"123", 3) + 2;
  win_width = XTextWidth(DispFont,"Break when Register", 18) + 2*offset_x;
  win_hieght = offset_y3 + 2* cell_y ;

  switch(i) {
    case 0:
      bkregWin = XCreateSimpleWindow(dpy, asmWin,
                KEYBOARD_OFFSET_X +  buttons_menu[MBUTTON_BREG].x,
                TOP_SKIP +  buttons_menu[MBUTTON_BREG].y ,
                win_width,win_hieght, 5,
                COLOR(BLACK), COLOR(DISP_PAD_TOP));
      mask =  CWEventMask | CWBackingStore;
      stwa.event_mask = KeyPressMask | KeyReleaseMask | ExposureMask |
                  KeymapStateMask | EnterWindowMask | StructureNotifyMask;
      stwa.backing_store = Always;
      XChangeWindowAttributes(dpy, bkregWin, mask, &stwa);
      XSetBackground(dpy, gc_stack, COLOR(DISP_PAD_TOP));
      XMapRaised(dpy, bkregWin);

      sprintf(buf,"Break when Register");
      XDrawImageString(dpy, bkregWin, gc_stack,
           10,font_height, buf, (int)strlen(buf));

      XFillRectangle(dpy, bkregWin, gc_stack_erase,
             offset_x, offset_y1,
             cell_x, cell_y) ;
      XDrawRectangle(dpy, bkregWin, gc_stack,
             offset_x-1, offset_y1,
             cell_x,cell_y) ;
      XDrawRectangle(dpy, bkregWin, gc_stack,
             offset_x-2, offset_y1-1,
             cell_x+2,cell_y+2) ;
    ...
    case 127:
      break;
    case 13:
      switch (buttons_menu[MBUTTON_BREG].code) {
        case 1:
          buttons_menu[MBUTTON_BREG].code = 2;
          break;
        case 2:
          buttons_menu[MBUTTON_BREG].code = 4;
          break;
        case 4:
          break_register (BreakAddr,  BKvalue, BKmask);
          XUnmapWindow(dpy, bkregWin);
          XDestroyWindow(dpy, bkregWin); 
          buttons_menu[MBUTTON_BREG].code = 0;
          break;
      }
      break;
    default:
      switch (buttons_menu[MBUTTON_BREG].code) {
        case 1:
          ...
          BKreg[j+1]=0;
          k = 0;
          for (op = bkreg; op->name != NULL; op++ ) {
            if ( !strcmp(op->name, BKreg)) {
              XSetBackground(dpy, gc_stack, COLOR(LCD));
              sprintf(buf,"%s",BKreg);
              XDrawImageString(dpy, bkregWin, gc_stack,
                 offset_x,offset_y1 + font_height, buf, (int)strlen(buf)); 
              break;
            }
            k++;
          }
          if ( op->name == NULL )
            ...
            BreakAddr = k;
            cell_x = XTextWidth(DispFont,"0x0000000000000000", op->len+2) + 2;
            RangeAddr = op->len;
            XFillRectangle(dpy, bkregWin, gc_stack_erase,
                 offset_x, offset_y2, cell_x, cell_y) ;
            XDrawRectangle(dpy, bkregWin, gc_stack,
                 offset_x-1, offset_y2, cell_x,cell_y) ;
            XDrawRectangle(dpy, bkregWin, gc_stack,
                 offset_x-2, offset_y2-1, cell_x+2,cell_y+2) ;

            XFillRectangle(dpy, bkregWin, gc_stack_erase,
                 offset_x, offset_y3, cell_x, cell_y) ;
            XDrawRectangle(dpy, bkregWin, gc_stack,
                 offset_x-1, offset_y3, cell_x,cell_y) ;
            XDrawRectangle(dpy, bkregWin, gc_stack,
                 offset_x-2, offset_y3-1, cell_x+2,cell_y+2) ;

            XSetForeground(dpy, gc_stack, COLOR(DISP_PAD_TOP));
            XFillRectangle(dpy, bkregWin, gc_stack,
                 offset_x + cell_x +1 , offset_y2-1,
                 win_width - (offset_x + cell_x +1) , cell_y + 3);
            XFillRectangle(dpy, bkregWin, gc_stack,
                 offset_x + cell_x +1 , offset_y3-1,
                 win_width - (offset_x + cell_x +1) , cell_y + 3);
            XSetForeground(dpy, gc_stack, COLOR(BLACK));
        ...
        case 2:
          ...
          XSetBackground(dpy, gc_stack, COLOR(LCD));
          if (RangeAddr <= 8) {
            BKmask.lo = (BKmask.lo << 4) + i;
            BKmask.lo &= 0xffffffff >> ((8-RangeAddr) *4);
            sprintf(fmt, "0x%%.%lXlX", RangeAddr );
            sprintf(buf, fmt, BKmask.lo);
          } else {
            BKmask.hi = (BKmask.hi << 4) + (BKmask.lo >> 28);
            BKmask.lo = (BKmask.lo << 4) + i;
            sprintf(buf,"0x%.8lX%.8lX", BKmask.hi, BKmask.lo);
          }
          XDrawImageString(dpy, bkregWin, gc_stack,
             offset_x,offset_y2 + font_height, buf, (int)strlen(buf));
          break;
        case 4:
          i -= 48;
          if (i > 9)
          ...
          XSetBackground(dpy, gc_stack, COLOR(LCD));
          if (RangeAddr <= 8) {
            BKvalue.lo = (BKvalue.lo << 4) + i;
            BKvalue.lo &= 0xffffffff >> ((8-RangeAddr) *4);
            sprintf(fmt, "0x%%.%lXlX", RangeAddr );
            sprintf(buf, fmt, BKvalue.lo);
          } else {
            BKvalue.hi = (BKvalue.hi << 4) + (BKvalue.lo >> 28);
            BKvalue.lo = (BKvalue.lo << 4) + i;
            sprintf(buf,"0x%.8lX%.8lX", BKvalue.hi, BKvalue.lo);
          }
          XDrawImageString(dpy, bkregWin, gc_stack,
             offset_x,offset_y3 + font_height, buf, (int)strlen(buf));
          break;
...
/*
      ...
      if (i > 15 || i < 0 )
        break;
      XSetBackground(dpy, gc_stack, COLOR(LCD));
      if (buttons_menu[MBUTTON_BRAG].code & 0x2 ){
        RangeAddr = RangeAddr << 4;
        RangeAddr += i;
        RangeAddr &= 0xfffff;
        sprintf(buf,"0x%.5lX", RangeAddr);
        XDrawImageString(dpy, bkregWin, gc_stack,
           offset_x,offset_y2 + font_height, buf, (int)strlen(buf));
      } else {
...
{
  char buf[100];
  int  offset_x, offset_y1, offset_y2, cell_x, cell_y, win_width, win_hieght;
  unsigned int  mask;
  XSetWindowAttributes stwa;
  ...
  offset_y2 = 3.5* font_height + DispFont->descent;
  cell_y = font_height + DispFont->descent;
  cell_x = XTextWidth(DispFont,"0x12345", 7) + 2;
  win_width = XTextWidth(DispFont,"Break at Range", 14) + 2*offset_x;
  win_hieght = offset_y2 + 2* cell_y ;

  switch(i) {
    case 0:
      bkrangeWin = XCreateSimpleWindow(dpy, asmWin,
                KEYBOARD_OFFSET_X +  buttons_menu[MBUTTON_BRAG].x,
                TOP_SKIP +  buttons_menu[MBUTTON_BRAG].y ,
                win_width,win_hieght, 5,
                COLOR(BLACK), COLOR(DISP_PAD_TOP));
      mask =  CWEventMask | CWBackingStore;
      stwa.event_mask = KeyPressMask | KeyReleaseMask | ExposureMask |
                  KeymapStateMask | EnterWindowMask | StructureNotifyMask;
      stwa.backing_store = Always;
      XChangeWindowAttributes(dpy, bkrangeWin, mask, &stwa);
      XSetBackground(dpy, gc_stack, COLOR(DISP_PAD_TOP));
      XSetForeground(dpy, gc_stack, COLOR(BLACK));
      XMapRaised(dpy, bkrangeWin);

      sprintf(buf,"Break at Range");
      XDrawImageString(dpy, bkrangeWin, gc_stack,
           10,font_height, buf, (int)strlen(buf));
      sprintf(buf,"0x00000");

      XFillRectangle(dpy, bkrangeWin, gc_stack_erase,
             offset_x, offset_y1,
             cell_x, cell_y) ;
      XDrawRectangle(dpy, bkrangeWin, gc_stack,
             offset_x-1, offset_y1,
             cell_x,cell_y) ;
      XDrawRectangle(dpy, bkrangeWin, gc_stack,
             offset_x-2, offset_y1-1,
      ...
      XFillRectangle(dpy, bkrangeWin, gc_stack_erase,
             ...
             cell_x, cell_y) ;
      XDrawRectangle(dpy, bkrangeWin, gc_stack,
             offset_x-1, offset_y2,
             cell_x,cell_y) ;
      XDrawRectangle(dpy, bkrangeWin, gc_stack,
             offset_x-2, offset_y2-1,
             cell_x+2,cell_y+2) ;
      XSetBackground(dpy, gc_stack, COLOR(LCD));
      XDrawImageString(dpy, bkrangeWin, gc_stack,
           offset_x,offset_y2 + font_height, buf, (int)strlen(buf));
    ...
    case 13:
      if ( buttons_menu[MBUTTON_BRAG].code & 0x2 ) {
        XUnmapWindow(dpy, bkrangeWin);
        XDestroyWindow(dpy, bkrangeWin); 
        break_range(BreakAddr,RangeAddr); 
        buttons_menu[MBUTTON_BRAG].code = 0;
      } else 
         buttons_menu[MBUTTON_BRAG].code = 2;
      break;
    default:
      i -= 48;
      if (i > 9)
...
{
  char buf[100];
  int  offset_x, offset_y, cell_x, cell_y, win_width, win_hieght;
  unsigned int  mask;
  XSetWindowAttributes stwa;

  offset_x = (int)SIDE_SKIP;
  offset_y = 1.5* font_height + DispFont->descent;
  cell_y = font_height + DispFont->descent;
  cell_x = XTextWidth(DispFont,"0x12345", 7) + 2;
  win_width = XTextWidth(DispFont,"Break at Address", 16) + 2*offset_x;
  win_hieght = offset_y + 2* cell_y ;

  switch(i) {
    case 0:
      bkaddressWin = XCreateSimpleWindow(dpy, asmWin, 
                KEYBOARD_OFFSET_X +  buttons_menu[MBUTTON_BADD].x, 
                TOP_SKIP +  buttons_menu[MBUTTON_BADD].y,
                win_width,win_hieght, 5,
                COLOR(BLACK), COLOR(DISP_PAD_TOP));
      mask =  CWEventMask | CWBackingStore;
      stwa.event_mask = KeyPressMask | KeyReleaseMask | ExposureMask |
                  KeymapStateMask | EnterWindowMask | StructureNotifyMask;
      stwa.backing_store = Always;
      XChangeWindowAttributes(dpy, bkaddressWin, mask, &stwa);
      XSetForeground(dpy, gc_stack, COLOR(BLACK));
      XSetBackground(dpy, gc_stack, COLOR(DISP_PAD_TOP));
      sprintf(buf,"Break at Address");
      XMapRaised(dpy, bkaddressWin);
      XDrawImageString(dpy, bkaddressWin, gc_stack,
           10,font_height, buf, (int)strlen(buf));
      XFillRectangle(dpy, bkaddressWin, gc_stack_erase,
             offset_x, offset_y,
             cell_x, cell_y) ;
      XDrawRectangle(dpy, bkaddressWin, gc_stack,
             offset_x-1, offset_y,
             cell_x,cell_y) ;
      XDrawRectangle(dpy, bkaddressWin, gc_stack,
      ...
      XSetBackground(dpy, gc_stack, COLOR(LCD));
      XDrawImageString(dpy, bkaddressWin, gc_stack,
           offset_x,offset_y + font_height, buf, (int)strlen(buf));
      BreakAddr = 0;
      break;
    case 8:
    case 127:
      ...
      break;
    case 13:
      XUnmapWindow(dpy, bkaddressWin);
      XDestroyWindow(dpy, bkaddressWin); 
      break_address(BreakAddr); 
      buttons_menu[MBUTTON_BADD].code = 0;
      break;
    default:
      i -= 48;
      if (i > 9)
...
#endif
{
  ...
  switch (i) {
    ...
    case MBUTTON_BNOW:
      ...
      break;
    case MBUTTON_BADD:
      if (!buttons_menu[MBUTTON_BADD].code) {
        buttons_menu[MBUTTON_BADD].code = 1;
        PopUpBreakAddress(0);
      }
      break;
    case MBUTTON_BRAG:
      if (!buttons_menu[MBUTTON_BRAG].code) {
        buttons_menu[MBUTTON_BRAG].code = 1;
        PopUpBreakRange(0);
      }
      break;
    case MBUTTON_BREG:
      if (!buttons_menu[MBUTTON_BREG].code) {
        buttons_menu[MBUTTON_BREG].code = 1;
        PopUpBreakRegister(0);
      }
      break;
    case MBUTTON_ASMTRAS:
      if (buttons_menu[MBUTTON_ASMTRAS].code) {
        Trace = 0;
        buttons_menu[MBUTTON_ASMTRAS].code = 0;
      } else {
        Trace = 1;
        buttons_menu[MBUTTON_ASMTRAS].code = 1;
      }
      break;
    case MBUTTON_ASMrplMODE:
      if (buttons_menu[MBUTTON_ASMrplMODE].code) {
        buttons_menu[MBUTTON_ASMrplMODE].code = 0;
        disassembler_mode = SYSRPL_MNEMONICS;
      } else {
        buttons_menu[MBUTTON_ASMrplMODE].code = 1;
        disassembler_mode = USERRPL_MNEMONICS;
      }
      update_instr();
      break;
    case MBUTTON_ASMasmMODE:
      if (buttons_menu[MBUTTON_ASMasmMODE].code) {
        buttons_menu[MBUTTON_ASMasmMODE].code = 0;
        disassembler_mode = CLASS_MNEMONICS;
      } else {
        buttons_menu[MBUTTON_ASMasmMODE].code = 1;
...
#endif
{
  ...
  switch (i) {
    ...
    case MBUTTON_GOR:
      ...
      on_event();
      break;
    case MBUTTON_MAINQUIT:
      debug();
      break;
    case MBUTTON_MAINEXIT:
        XCloseDisplay(dpy);
        exit (0);
      break;
    case MBUTTON_DEBUG:
      if (!Disassembly) {
        Disassembly = 1;
        XMapWindow(dpy, asmWin);
        XMapSubwindows(dpy, asmWin);
        update_instr();
      }
      else {
        Disassembly = 0;
        XUnmapSubwindows(dpy, asmWin);
        XUnmapWindow(dpy, asmWin);
      }
      break;
    case MBUTTON_LOBJ:
      if (!Object) {
        Object = 1;
        XMapWindow(dpy, objWin);
        XMapSubwindows(dpy, objWin);
        update_instr();
      }
      else {
        Object = 0;
        XUnmapSubwindows(dpy, objWin);
        XUnmapWindow(dpy, objWin);
      }
      break;
    case MBUTTON_LRAM:
      if (!Memory)
        {
          Memory = 1;
          XMapWindow(dpy, ramWin );
          XMapSubwindows(dpy, ramWin);
          update_memory();
        }
      else
        {
           Memory = 0;
           XUnmapSubwindows(dpy, ramWin);
           XUnmapWindow(dpy, ramWin);
        }
      break;
    case MBUTTON_LREG:
      if (!Register)
        {
          Register = 1;
          XMapWindow(dpy, regWin );
          XMapSubwindows(dpy, regWin);
        }
      else
        {
           Register = 0;
           XUnmapSubwindows(dpy, regWin);
           XUnmapWindow(dpy, regWin);
        }
      break;
    case MBUTTON_LSTK:
      if(!Stack)
        {
          Stack = 1;
          XMapWindow(dpy, stackWin );
          XMapSubwindows(dpy, stackWin);
          update_gui();
        }
      else
        {
           Stack = 0;
           XUnmapSubwindows(dpy, stackWin);
           XUnmapWindow(dpy, stackWin);
...
#endif
{
  ...
  while (XPending(dpy) > 0) {
    ...
    switch ((int)xev.type) {
      ...
      case KeyPress:
        if (xev.xkey.window == mainW) {
          ...
          i = XLookupString(&xev.xkey, buf, bufs, &sym, NULL);
          if ( xev.xkey.window == bkaddressWin && buf[0] )
            PopUpBreakAddress(buf[0]);
          if ( xev.xkey.window == bkrangeWin && buf[0] )
            PopUpBreakRange(buf[0]);
          if ( xev.xkey.window == bkregWin && buf[0] )
            PopUpBreakRegister(buf[0]);
        }
        break;
	...
	break;
      ...
      case UnmapNotify:
        if(xev.xunmap.window == mainW)
          disp.mapped = 0;
        break;

      case MapNotify:
        if(xev.xmap.window == mainW)
          disp.mapped = 1;
        update_display();
        redraw_annunc();
        break;
	  ...
	  for (i = BUTTON_A; i <= LAST_BUTTON; i++) {
	    if (xev.xbutton.subwindow == buttons[i].xwin) {
          ...
          for (i = MBUTTON_START; i <= MBUTTON_DONE; i++) {
            if (xev.xbutton.subwindow == buttons_menu[i].xwin) {
              buttons_menu[i].key_down = 1;
              buttons_menu[i].pressed = 1;
              wake = 1;
              DrawButton(i,buttons_menu);
              if (i >= MBUTTON_MENUSTART && i <= MBUTTON_MENUDONE )
                MainMenu(i);
              if (i >= MBUTTON_REGSTART && i <= MBUTTON_REGDONE )
                RegMenu(i);
              if (i >= MBUTTON_STSTART && i <= MBUTTON_STDONE )
                StackMenu(i);
              if (i >=  MBUTTON_ASMSTART && i<= MBUTTON_ASMDONE )
                AssMenu(i);
              if (i >= MBUTTON_RAMSTART && i <= MBUTTON_RAMDONE )
                RamMenu(i, xev.xbutton.button);
              if (i >= MBUTTON_OBJSTART && i <= MBUTTON_OBJDONE )
                ObjectMenu(i);
              break;
            }
          }
          if ( i >= MBUTTON_DONE ) {
            if (xev.xbutton.window == objWin)
              update_object(xev.xbutton.y);
          }
        }
        ...
        if (opt_gx)
            ...
            i = SIDE_SKIP;
        if ( xev.xbutton.x > i  && xev.xbutton.x < i + hp_width &&
               xev.xbutton.y > 10 && xev.xbutton.y < 10 + hp_height)
          {
            if (xev.xbutton.window == mainW)
              PopUpMenu(); 
          }
	...
	for (i = BUTTON_A; i <= LAST_BUTTON; i++) {
	  if (xev.xbutton.subwindow == buttons[i].xwin) {
        ...
        for (i = MBUTTON_START; i <= MBUTTON_DONE; i++) {
          if (xev.xbutton.subwindow == buttons_menu[i].xwin || buttons_menu[i].pressed) {
            buttons_menu[i].key_down = 0;
            if (xev.xbutton.button == Button1) {
              wake = 1;
              buttons_menu[i].pressed = 0;
              DrawButton(i, buttons_menu);
              break;
            }
          }
	...
	switch (xev.xmapping.request) 
	{
      ...
      case EnterNotify:
	    ...
	    if (button_leave >= 0) {
              ...
              wake = 1;
              DrawButton(button_leave,buttons);
	      button_leave = -1;
            }
          }
        }
