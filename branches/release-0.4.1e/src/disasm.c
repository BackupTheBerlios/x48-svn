...
char *
#ifdef __FunctionProto__
append_word (char *buf, word_20 addr)
#else
append_word (buf, addr)
     char *buf;
     word_20 addr;
#endif
{
	...
	default:
	  ...
	  break;
        ...
        if(dis_reg) {
          ...
          if (n & 1) {
            p = append_str (p, "D1=");
            p = append_word(p, saturn.D1);
          }
          else {
            p = append_str (p,"D0=");
            p = append_word(p, saturn.D0);
          }
        }
      break;
	  ...
	  if (NULL != (p = in_str_808[fn + 16 * disassembler_mode]))
	    {
	      p = append_str (out, p);
              if(fn == 0xC) {
                ...
                p = append_tab(out);
                p = append_tab(out);
                dump_reg(";A=", 5, saturn.A, reg);
                p = append_str (p, reg);
                if ( (k = match_rom( saturn.A[0] +
                           (saturn.A[1] << 4) + (saturn.A[2] << 8) +
                           (saturn.A[3] << 12) + (saturn.A[4] << 16) )) != 0) {
                 p = append_str(p, "  ");
                 p = append_str(p, saturn.entries[k].defn);
                }
              }
	      return p;
	    }
...
word_20
#ifdef __FunctionProto__
disassemble (word_20 addr, char *out, int full)
#else
disassemble (addr, out, full)
     word_20 addr;
     char *out;
     int full;
#endif
{
  ...
  dis_reg = full;
  if (disassembler_mode ==  SYSRPL_MNEMONICS)
    {
       i = 0;
       strcpy(out, "nope\0");
       while ( i < 2999 && saturn.entries[i].rom != addr )
         i++;
       if ( i < 2990 ) 
         strcpy (out,saturn.entries[i].defn); 
    }
  else
    {
      switch (n = read_nibble (addr++))
        ...
        if (dis_reg) {
          ...
          if (pc == 0) {
            switch (n) {
              ...
              default:
                p = append_str(p, "n=");
                p = append_word(p, (word_20)n);
                p = append_str(p, "    pc=");
                p = append_word(p, (word_20)pc);
                break;
            }
          }
          else {
